...
public class AddressBook {
    ...
    private static final String MESSAGE_EXITING_ADDFAV_COMMAND = "Exiting addfav command";
    ...
    private static String executeAddFav() {
        ...
        if (ALL_PERSONS.size() == 0) {
            return MESSAGE_NO_PERSON_IN_ADDRESSBOOK;
        }
        // get the target index from user input
        System.out.print("|| Enter index of person to add (0 to exit): ");
        String indexString = SCANNER.next();
        if (!isIndexValid(indexString)){ // if index is invalid
            return MESSAGE_INVALID_PERSON_DISPLAYED_INDEX;
        }
        int indexToAdd = getIndexToAdd(indexString);
        if (indexToAdd == 0){
            return MESSAGE_EXITING_ADDFAV_COMMAND;
        }
     ...
     *
     * @param indexString index string from user input
     * @return an index integer
     */
    private static int getIndexToAdd(String indexString) {
        int indexToAdd = Integer.parseInt(indexString);
        System.out.println("||");
        return indexToAdd;
    }

    /**
     * Checks validity of index string's format.
     *
     * @param indexString index string from user input
     * @return whether the input string is valid
     */
    private static boolean isIndexValid(String indexString) {
        try {
    ...
    /**
     * Clears all persons in the favourites list.
     *
     * @return feedback display message for the operation result
     */
    ...
    */
    ...
    private static boolean deletePersonFromAddressBook(HashMap<PersonProperty, String> exactPerson) {
        ...
        final boolean changed2 = FAV_LIST.remove(exactPerson);
        if (changed) {
            savePersonsToFile(getAllPersonsInAddressBook(), storageFilePath);
        }
        if (changed2) {
